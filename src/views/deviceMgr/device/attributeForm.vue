...
<template>
  <el-form ref="dialogForm" :rules="rules" :model="formData" label-width="100px" class="attribute-form">
    <el-form-item label="属性名称" prop="name">
      <el-input v-model="formData.name" size="mini" :disabled="disabled" />
    </el-form-item>
...
export default {
  ...
  methods: {
    ...
    validateForm() {
      ...
      return flag
      // && this.$refs.pretreatment.verification() && this.$refs.tag.verification()
    },
    sourceChange(val) {
      this.$refs.dialogForm.clearValidate('dependencyAttrId')
